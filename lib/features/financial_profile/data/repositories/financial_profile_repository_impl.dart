...
class FinancialProfileRepositoryImpl implements FinancialProfileRepository {
  ...
  Future<Either<Failure, List<Question>>> getQuestions() async {
    try {
      // Les questions pourraient venir d'un JSON local ou de l'API.
      // Si remoteDataSource est null, on devrait avoir un fallback.
      // Pour l'instant, on suppose que remoteDataSource gère ça.
      if (remoteDataSource != null) {
        final questionsModels = await remoteDataSource!.getQuestions();
        final questions = questionsModels.map((model) => model.toEntity()).toList();
        questions.sort((a, b) => a.order.compareTo(b.order));
        return Right(questions);
      }
      return Left(ServerFailure('DataSource non configurée'));
    } catch (e) {
