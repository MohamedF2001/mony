...
class CategoryTrendData {
  final DateTime date;
  final Map<String, double> categories; // CategoryName -> Amount

  CategoryTrendData({required this.date, required this.categories});
}

final categoryTrendProvider = Provider<AsyncValue<List<CategoryTrendData>>>((ref) {
  final transactions = ref.watch(periodFilteredTransactionsProvider);
  final statisticsState = ref.watch(statisticsProvider);

  if (transactions.isEmpty) return const AsyncValue.data([]);
  ...
  for (var t in transactions) {
    ...
    String key;
    DateTime date;
    if (statisticsState.period == StatisticsPeriod.year) {
      key = '${t.date.year}-${t.date.month}';
      date = DateTime(t.date.year, t.date.month, 1);
    } else if (statisticsState.period == StatisticsPeriod.month) {
      final week = _getWeekNumber(t.date);
      key = '${t.date.year}-${t.date.month}-W$week';
      date = _getStartOfWeek(t.date);
    } else {
      key = '${t.date.year}-${t.date.month}-${t.date.day}';
  ...
  final result = sortedKeys.map((k) {
    ...
    final date = DateTime.fromMillisecondsSinceEpoch(data['_date']!.toInt());
    final cats = Map<String, double>.from(data)..remove('_date');
    return CategoryTrendData(date: date, categories: cats);
  }).toList();
